...
# Global variable to store the last deleted expense
last_deleted_expense = None

@app.route('/delete_expense/<int:expense_id>', methods=["GET"])
def delete_expense(expense_id):
    global last_deleted_expense

    cursor.execute("SELECT * FROM expenses WHERE expense_id=%s", (expense_id,))
    last_deleted_expense = cursor.fetchone()

    if last_deleted_expense:
        cursor.execute("DELETE FROM expenses WHERE expense_id=%s", (expense_id,))
        connect_.commit()
        return render_template('index.html', rollback_available=True)
    else:
        return jsonify({"error": "Expense not found."})


@app.route('/rollback_deletion', methods=["GET"])
def rollback_deletion():
    global last_deleted_expense

    if last_deleted_expense:
        # Reinsert the deleted expense
        query = """INSERT INTO expenses 
                   (expense_id, user_id, category_id, date, amount, family_id, description, receipt) 
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
        cursor.execute(query, last_deleted_expense)
        connect_.commit()
        last_deleted_expense = None  
        return redirect(url_for('index'))
    else:
        return jsonify({"error": "No recent deletion to rollback."})
